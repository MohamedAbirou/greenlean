...
const ChallengeForm: React.FC<ChallengeFormProps> = ({
  ...
  useEffect(() => {
    if (challenge) {
      ...
      setRequirementsJson(JSON.stringify(challenge.requirements ?? { target: 0 }, null, 2));
    } else {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <Select
              value={formData.type}
              onValueChange={(value) => setFormData({ ...formData, type: value as ChallengeType })}
            >
          ...
          <div>
            ...
            >
              ...
              <SelectContent>
                ...
                {badges.map((badge) => {
                  const BadgeIconComponent = IconMap[badge?.icon ?? "star"] || LucideIcons.Star;
                  return (
                    <SelectItem
                      key={badge.id}
                      value={badge.id}
                    ...
                    >
                      {BadgeIconComponent && (
                        <BadgeIconComponent className="w-5 h-5" style={{ color: badge.color }} />
                      )}
