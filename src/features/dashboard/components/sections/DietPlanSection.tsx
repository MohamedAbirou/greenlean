...
export const DietPlanSection: React.FC<DietPlanSectionProps> = memo(({ userId, calculations }) => {
  ...
  const handleRegeneratePlan = async () => {
    try {
      ...
      toast.info("Regenerating your meal plan...");
      ...
      toast.success("Meal plan regeneration started! This will take about 45 seconds.");
    } catch (error) {
      ...
      <div className="mx-auto space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-4 mb-6">
          <div>
            ...
            <p className="text-muted-foreground">AI is crafting your custom nutrition plan...</p>
          </div>
        </div>
        <GeneratingState type="meal" startedAt={new Date()} />
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="px-4 py-3 bg-white dark:bg-slate-800 border-2 border-emerald-500 text-emerald-600 dark:text-emerald-400 rounded-md font-semibold shadow hover:shadow-lg transform hover:scale-105 transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
          >
