...
export const WorkoutSection: React.FC<WorkoutSectionProps> = memo(({ userId }) => {
  ...
  const handleRegeneratePlan = useCallback(async () => {
    try {
      ...
      sonnerToast.info("Regenerating your workout plan...");

      await quizApi.retryPlanGeneration(userId);

      // Invalidate queries to show generating state
      queryClient.invalidateQueries({ queryKey: ["workoutPlan", userId] });

      sonnerToast.success("Workout plan regeneration started! This will take about 40 seconds.");
    } catch (error) {
      console.error("Failed to regenerate plan:", error);
      sonnerToast.error("Failed to regenerate plan. Please try again.");
    } finally {
  ...
  // AI Generation state - show professional progress UI
  if (isGenerating) {
    return (
      <div className="mx-auto space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-4 mb-6">
          <div>
            ...
            <p className="text-muted-foreground">AI is designing your training program...</p>
          </div>
        </div>
        <GeneratingState type="workout" startedAt={new Date()} />
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="px-4 py-3 bg-white dark:bg-slate-800 border-2 border-indigo-500 text-indigo-600 dark:text-indigo-400 rounded-md font-semibold shadow hover:shadow-lg transform hover:scale-105 transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
          >
