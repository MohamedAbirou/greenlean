...
export function generateMealPlan(
  ...
  const templates =
    mealTemplates[normalizedDietType] || mealTemplates["omnivore"];
  ...
  const distribution = getSmartCalorieDistribution(
    validMealsPerDay,
    goal,
    activityLevel
  );
...
function getFilteredTemplates(
  ...
  filteredTemplates = filteredTemplates.filter(
    (template) => !usedTemplates.has(template.name || "")
  );
  ...
  template.items.forEach((item: any) => {
    ...
    if (food.healthBenefits) {
      food.healthBenefits.forEach((benefit) => {
        ...
        if (healthConditions.heartDisease && benefit === "heart disease")
          score += 2;
        if (
          healthConditions.highBloodPressure &&
          benefit === "high blood pressure"
        )
          score += 2;
      });
    }
